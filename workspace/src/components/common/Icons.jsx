import { 
    Home as HomeIcon, 
    Workflow,
    Briefcase as BriefcaseIcon,
    FileText as FileTextIcon,
    GraduationCap,
    Store as StoreIcon,
    MessageSquare,
    Users as UsersIcon,
    LayoutDashboard,
    Settings as SettingsIcon,
    Trophy as TrophyIcon,
    Crown as CrownIcon,
    Gem,
    Rocket as RocketIcon,
    Zap,
    Brain as BrainIcon,
    Gift as GiftIcon,
    Menu as MenuIcon,
    X,
    Check as CheckIcon,
    Folder as FolderIcon,
    Gamepad2,
    User as UserIcon,
    Sprout,
    Star as StarIcon,
    Target as TargetIcon,
    Building2,
    Medal as MedalIcon,
    Lock as LockIcon,
    AlertTriangle,
    Info as InfoIcon,
    Shield as ShieldIcon,
    Calendar as CalendarIcon,
    Swords as SwordsIcon,
    Lightbulb,
    Wrench as WrenchIcon,
    Bomb as BombIcon,
    LogOut as LogOutIcon,
    Code as CodeIcon,
    Palette as PaletteIcon,
    BarChart,
    Paperclip as PaperclipIcon,
    Send as SendIcon,
    ArrowLeft as ArrowLeftIcon,
    ArrowRight as ArrowRightIcon,
    Pencil as PencilIcon,
    Plus as PlusIcon,
    List as ListIcon,
    Eye as EyeIcon,
    Link as LinkIcon
} from 'lucide-react';

export const Home = HomeIcon;
export const Workflows = Workflow;
export const Briefcase = BriefcaseIcon;
export const Documents = FileTextIcon;
export const Training = GraduationCap;
export const Store = StoreIcon;
export const Chat = MessageSquare;
export const Team = UsersIcon;
export const Dashboard = LayoutDashboard;
export const Settings = SettingsIcon;
export const Trophy = TrophyIcon;
export const Crown = CrownIcon;
export const Diamond = Gem;
export const Rocket = RocketIcon;
export const Lightning = Zap;
export const Brain = BrainIcon;
export const Gift = GiftIcon;
export const Menu = MenuIcon;
export const Close = X;
export const Check = CheckIcon;
export const Users = UsersIcon;
export const AcademicCap = GraduationCap;
export const Folder = FolderIcon;
export const GameController = Gamepad2;
export const User = UserIcon;
export const Seed = Sprout;
export const Star = StarIcon;
export const Target = TargetIcon;
export const Building = Building2;
export const Medal = MedalIcon;
export const Lock = LockIcon;
export const Exclamation = AlertTriangle;
export const Info = InfoIcon;
export const Shield = ShieldIcon;
export const Calendar = CalendarIcon;
export const Swords = SwordsIcon;
export const LightBulb = Lightbulb;
export const Wrench = WrenchIcon;
export const Bomb = BombIcon;
export const LogOut = LogOutIcon;
export const Code = CodeIcon;
export const Palette = PaletteIcon;
export const Chart = BarChart;
export const Paperclip = PaperclipIcon;
export const Send = SendIcon;
export const ArrowLeft = ArrowLeftIcon;
export const ArrowRight = ArrowRightIcon;
export const Pencil = PencilIcon;
export const Plus = PlusIcon;
export const List = ListIcon;
export const Eye = EyeIcon;
export const Link = LinkIcon;
export const FileText = FileTextIcon;

export const Icons = {
    Home,
    Workflows,
    Briefcase,
    Documents,
    Training,
    Store,
    Chat,
    Team,
    Dashboard,
    Settings,
    Trophy,
    Crown,
    Diamond,
    Rocket,
    Lightning,
    Brain,
    Gift,
    Menu,
    Close,
    Check,
    Users,
    AcademicCap,
    Folder,
    GameController,
    User,
    Seed,
    Star,
    Target,
    Building,
    Medal,
    Lock,
    Exclamation,
    Info,
    Shield,
    Calendar,
    Swords,
    LightBulb,
    Wrench,
    Bomb,
    LogOut,
    Code,
    Palette,
    Chart,
    Paperclip,
    Send,
    ArrowLeft,
    ArrowRight,
    Pencil,
    Plus,
    List,
    Eye,
    Link,
    FileText
};
