        ...
        <div className={`flex items-end gap-2 ${isMe ? 'justify-end' : 'justify-start'}`}>
            ...
            {!isMe && (
                <Link to={`/users/${message.user_id}`} className="flex-shrink-0">
                    <div className="h-8 w-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold text-sm hover:ring-2 hover:ring-primary-500 transition-all">
                        {senderInitial}
                    </div>
                </Link>
            )}
            ...
            {/* Message Bubble */}
            <div className={`max-w-[75%] sm:max-w-[70%] rounded-lg px-3 py-2 shadow-sm ${isMe
                ? 'bg-gradient-to-br from-primary-600 to-primary-700 text-white rounded-br-none border border-primary-500/50'
                : 'bg-white/10 backdrop-blur-md text-gray-100 border border-white/10 rounded-bl-none'
                }`}>
                {!isMe && (
                    <Link to={`/users/${message.user_id}`}>
                        <div className="text-xs font-semibold text-primary-300 mb-1 hover:underline">
                    ...
                    <div className="mb-2">
                        {message.attachment_type?.startsWith('image/') ? (
                            ...
                            <a
                                ...
                                className="flex items-center space-x-2 p-2 rounded bg-black/20 hover:bg-black/30 transition-colors border border-white/10"
                            >
